        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (GetAdminInfo().role_id != 1)
                {
                    ...
                    {
                        RptBind(" DATENAME(year,add_time)=" + DateTime.Now.Year + " and DATENAME(month,add_time)=" + DateTime.Now.Month + " and contract_status=0 and audit_stutas=1 and user_id=" + GetAdminInfo().id + CombSqlTxt(channel_id, starttime, endtime), "user_id", "datepart(year,add_time) , datepart(month,add_time) ,user_id");
                    }
                    else
                    {
                        RptBind(" audit_stutas=1 and contract_status=0 and user_id=" + GetAdminInfo().id + CombSqlTxt(channel_id, starttime, endtime), "user_id", "datepart(year,add_time) , datepart(month,add_time) ,user_id");
