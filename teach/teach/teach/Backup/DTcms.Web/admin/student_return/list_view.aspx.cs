    ...
    public partial class list_view : DTcms.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (channel_id==8)
                {
                    RptBind("id>0 and return_user_id in(select id from dt_manager where role_id=10)"  + CombSqlTxt(this.channel_id, this.category_id, this.keywords, this.property), "add_time desc");
                }
                else
                {
        ...
        protected string CombSqlTxt(int _channel_id, int _category_id, string _keywords, string _property)
        {
            ...
            {
                strTemp.Append(" and  return_user_id in (select id from dt_manager where real_name  like '%" + _keywords + "%')");
            }
