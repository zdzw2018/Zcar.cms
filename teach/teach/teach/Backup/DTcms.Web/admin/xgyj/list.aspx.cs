...
namespace DTcms.Web.admin.xgyj
{
    public partial class list : DTcms.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (GetAdminInfo().role_id != 1)
                {
                    ...
                    {
                        RptBind(" DATENAME(year,add_time)=" + DateTime.Now.Year + " and DATENAME(month,add_time)=" + DateTime.Now.Month + " and contract_status=1 and audit_stutas=1 and user_id=" + GetAdminInfo().id + CombSqlTxt(channel_id, starttime, endtime), "user_id", "datepart(year,add_time) , datepart(month,add_time) ,user_id");
                    }
                    else
                    {
                        RptBind(" audit_stutas=1 and contract_status=1 and user_id=" + GetAdminInfo().id + CombSqlTxt(channel_id, starttime, endtime), "user_id", "datepart(year,add_time) , datepart(month,add_time) ,user_id");
                ...
                else
                {
                    if (string.IsNullOrEmpty(endtime))
                    {
                        RptBind(" DATENAME(year,add_time)=" + DateTime.Now.Year + " and DATENAME(month,add_time)=" + DateTime.Now.Month + " and contract_status=1 and audit_stutas=1" + CombSqlTxt(channel_id, starttime, endtime), "user_id", "datepart(year,add_time) , datepart(month,add_time),user_id");
                    }
                    else
                    {
                        RptBind(" audit_stutas=1 and contract_status=1" + CombSqlTxt(channel_id, starttime, endtime), "user_id", "datepart(year,add_time) , datepart(month,add_time),user_id");
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("list.aspx", "channel_id={0}&starttime={1}&endtime={2}",
