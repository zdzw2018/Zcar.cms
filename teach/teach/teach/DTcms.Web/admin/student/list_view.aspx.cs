...
namespace DTcms.Web.admin.student
{
    public partial class list_view : DTcms.Web.UI.ManagePage
    {
        ...
        protected string CombSqlTxt(int _channel_id, int _category_id, string _keywords, string _property)
        {
            ...
            {
                strTemp.Append(" and contract_status=0 and stu_id in(select stu_id from tb_student_contract where audit_stutas=1) and stu_id in (select tb_student_teach.stu_id from tb_student_teach where tb_student_teach.manager_id=" + model.id + " )");
            }
            
            else
            {
                strTemp.Append(" and contract_status=0 and stu_id in(select stu_id from tb_student_contract where audit_stutas=1 and xiaoqu=" + model.xiaoqu + ")");
            }
            //if (_channel_id > 0)
            //{
        ...
        protected decimal getGiveTotalKeShi(int stuid)
        {
            DTcms.BLL.student_contract bll = new BLL.student_contract();
