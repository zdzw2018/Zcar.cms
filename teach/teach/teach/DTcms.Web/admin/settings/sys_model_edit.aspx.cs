...
{
    public partial class sys_model_edit : DTcms.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == ActionEnum.Edit.ToString())
            {
                    ...
                    if (!new DTcms.BLL.sys_model().Exists(this.id))
                    {
        ...
        {
            DTcms.BLL.sys_model bll = new BLL.sys_model();
            DTcms.Model.sys_model model = bll.GetModel(_id);
        ...
        {
            bool result = true;
            DTcms.Model.sys_model model = new Model.sys_model();
            DTcms.BLL.sys_model bll = new BLL.sys_model();
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("sys_model", ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("sys_model", ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
