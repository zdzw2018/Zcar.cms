...
namespace DTcms.Web.UI
{
    public class ManagePage : System.Web.UI.Page
    {
        ...
        public void ChkRole(int[] rolestr,int roleid)
        {
            int id = Array.IndexOf(rolestr,roleid);
            if (id == -1)
            {
                string msbox = "parent.f_errorTab(\"错误提示\", \"您没有管理该页面的权限，请勿尝试非法进入！\")";
                //ClientScript.RegisterClientScriptBlock(Page.GetType(), "JsPrint", msbox.ToString(), true);  //修正BUG
        ...
        /// <param name="channel_id">频道ID</param>
        /// <param name="action_type">操作类型</param>
        public bool ChkAdminLevel1(int channel_id, string action_type)
        {
            Model.manager model = GetAdminInfo();
            BLL.manager_role bll = new BLL.manager_role();
            return bll.Exists(model.role_id, channel_id, action_type);
        }

        public bool ValAdminLevel(int channel_id,string action_type)
        {
            Model.manager model = GetAdminInfo();
            BLL.manager_role bll = new BLL.manager_role();
            return bll.Exists(model.role_id, channel_id, action_type);
