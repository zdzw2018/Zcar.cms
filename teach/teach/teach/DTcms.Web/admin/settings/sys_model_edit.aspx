...
<script type="text/javascript">
                ...
                case 0:
                    $(this).click(function () {
                        ...
                        $.ligerDialog.confirm("确定要删除吗？", "提示信息", function (result) {
                            if (result) {
                                var nav_id = obj.parent().parent().find("input[name='nav_id']").attr("value"); //取得对应的ID值
                                if (nav_id > 0) {
                                    if (delData(nav_id) == true) {
    ...
    //AJAX删除数据
    function delData(navId) {
        var result = true;
        $.ajax({
            type: "post",
            dataType: "json",
            url: "../../tools/admin_ajax.ashx?action=sys_model_nav_del",
            data: {
                nav_id: function () {
                    return navId;
                }
