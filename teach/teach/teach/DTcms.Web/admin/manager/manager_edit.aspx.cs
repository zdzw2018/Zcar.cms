...
namespace DTcms.Web.admin.manager
{
    public partial class manager_edit : DTcms.Web.UI.ManagePage
    {
        ...
        private void ShowInfo(int _id)
        {
            ...
            rblIsJianZhi.SelectedValue = model.is_jianzhi.ToString();
            ...
            objectSite.SetChkListValue(txtlesson,model.lesson);
            ddlXiaoQu.SelectedValue = model.xiaoqu.ToString();
        }
        #endregion
        ...
        private bool DoAdd()
        {
            ...
            model.is_jianzhi = int.Parse(rblIsJianZhi.SelectedValue);
            model.add_time = DateTime.Now;
            model.grade = txtgrade.SelectedValue;
            model.lesson = objectSite.GetChkListValue(txtlesson);
            model.xiaoqu = int.Parse(ddlXiaoQu.SelectedValue);

            if (bll.Exists(txtUserName.Text.Trim()))
            {
                JscriptMsg("该用户名已经存在，请更换一个！", "", "Error");
                return false;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("sys_manager", ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("sys_manager", ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
