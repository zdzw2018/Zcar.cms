...
namespace DTcms.Web.admin.msg_income
{
    public partial class list : DTcms.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                RptBind("msg_income_del=0 and msg_income_user='" + GetAdminInfo().user_name + "'" + CombSqlTxt(this.channel_id, this.category_id, this.keywords, this.property), "add_time desc");
            }
        }
        ...
        protected string CombSqlTxt(int _channel_id, int _category_id, string _keywords, string _property)
        {
            ...
            {
                strTemp.Append(" and msg_title like '%" + _keywords + "%'");
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("message_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("message_page_size", _pagesize.ToString(), 43200);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("list.aspx", "channel_id={0}&category_id={1}&keywords={2}&property={3}",
