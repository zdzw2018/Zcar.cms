...
namespace DTcms.Web.admin.bmyj
{
    public partial class shangke : ManagePage
    {
        ...
        protected decimal total5;
        protected int totalhetong;
        protected int totalnewmoney;
        protected int totalrealmoney;
        ...
        public string getShengyu(int user_id)
        {
            ...
            DataTable dt = new BLL.student_contract().GetList(0, " audit_stutas=1 and stu_id in(select stu_id from tb_student_teach where manager_name=(select real_name from dt_manager where id=" + user_id + " ) and lesson='') and stu_id in(select id from tb_student_info)", " audit_date desc").Tables[0];
        ...
        private void RptBind(string _strWhere, string _groupBy)
        {
            ...
            for (int i = 0; i < listGroupBy.Tables[0].Rows.Count; i++)
            {
                this.totalnewmoney += int.Parse(listGroupBy.Tables[0].Rows[i]["stucount"].ToString());
                this.totalrealmoney += this.getNoKeStu(int.Parse(listGroupBy.Tables[0].Rows[i]["manager_id"].ToString()));
                this.totalhetong += this.getJieKeStu(int.Parse(listGroupBy.Tables[0].Rows[i]["manager_id"].ToString()));
                this.total4 += decimal.Parse(this.getShengyu(int.Parse(listGroupBy.Tables[0].Rows[i]["manager_id"].ToString())));
                this.total5 += decimal.Parse(this.getShengYuMoney(int.Parse(listGroupBy.Tables[0].Rows[i]["manager_id"].ToString())));
            }
