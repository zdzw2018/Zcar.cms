...
namespace DTcms.DAL
{
		...
		public partial class market_outfield
	{
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from tb_market_outfield");
			strSql.Append(" where ");
			                                       strSql.Append(" id = @id  ");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into tb_market_outfield(");			
            strSql.Append("activity_time,activity_location,activity_content,watchers,collect_msg,oprice_push,part_time_fees,ques_feed,add_time,user_id,channel_id,xiaoqu");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update tb_market_outfield set " + strValue);
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update tb_market_outfield set ");
...
SqlParameter[] parameters = {
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from tb_market_outfield ");
			strSql.Append(" where id=@id");
						SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		public Model.market_outfield GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id, activity_time, activity_location, activity_content, watchers, collect_msg, oprice_push, part_time_fees, ques_feed, add_time, user_id, channel_id,xiaoqu  ");			
			strSql.Append("  from tb_market_outfield ");
			strSql.Append(" where id=@id");
						SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		{
			StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM tb_market_outfield ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
		...
		{
			StringBuilder strSql = new StringBuilder();
            strSql.Append("select "+strSelect+" FROM tb_market_outfield ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
