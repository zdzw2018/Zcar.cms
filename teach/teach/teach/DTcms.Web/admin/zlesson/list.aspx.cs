    ...
    public partial class list : DTcms.Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (model.role_id != 1)
                {
                    RptBind("id>0  and  manager_id=" + model.id + " )" + CombSqlTxt(this.channel_id, this.category_id, this.keywords, this.property), "add_time desc");
                }
                else
                {
        ...
        protected string CombSqlTxt(int _channel_id, int _category_id, string _keywords, string _property)
        {
            ...
            {
                strTemp.Append(" and lesson_teach like '%" + _keywords + "%'");
            }
