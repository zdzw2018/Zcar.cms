...
namespace DTcms.Web.admin.student
{
    public partial class edit_fp : DTcms.Web.UI.ManagePage
    {
        ...
        {
            DataTable dt = new BLL.manager().GetList(string.Format("lesson like '%{0}%' and xiaoqu={1}",txtlesson.SelectedValue,GetAdminInfo().xiaoqu)).Tables[0];
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                if (action == ActionEnum.Edit.ToString()) //修改
                {
                    ...
                    JscriptMsg("修改学生信息成功啦！", "list_teach_lesson.aspx?channel_id=" + this.channel_id+"&user_id="+this.user_id+"&tpage="+tpage, "Success");
                }
                else //添加
                {
                    ...
                    JscriptMsg("添加学生信息成功啦！", "list_teach_lesson.aspx?channel_id=" + this.channel_id + "&user_id=" + this.user_id + "&tpage=" + tpage, "Success");
                }
            }
            catch (Exception)
            {
