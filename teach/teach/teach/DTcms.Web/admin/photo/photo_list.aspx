<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="photo_list.aspx.cs" Inherits="DTcms.Web.admin.photo.photo_list" %>
...
<form id="form1" runat="server">
    <div class="navigation">首页 &gt; 图文模块 &gt; 管理列表</div>
    <div class="tools_box">
	    <div class="tools_bar">
            ...
            <a href="photo_edit.aspx?action=<%=ActionEnum.Add %>&channel_id=<%=this.channel_id %>" class="tools_btn"><span><b class="add">添加图文</b></span></a>
    ...
    <ItemTemplate>
      <tr>
        ...
        <td><a href="photo_edit.aspx?channel_id=<%#this.channel_id %>&action=<%#ActionEnum.Edit %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></td>
        ...
        <td align="center"><a href="photo_edit.aspx?channel_id=<%#this.channel_id %>&action=<%#ActionEnum.Edit %>&id=<%#Eval("id")%>">修改</a></td>
      </tr>
    </ItemTemplate>
    <FooterTemplate>
    ...
    <ItemTemplate>
      <li>
        <div class="box">
          ...
          <a href="photo_edit.aspx?channel_id=<%#this.channel_id %>&action=<%#ActionEnum.Edit %>&id=<%#Eval("id")%>">
          <img src="<%#Eval("img_url")%>" width="96" height="96" class="cover" />
          </a>
          <dl>
            <dt><a href="photo_edit.aspx?channel_id=<%#this.channel_id %>&action=<%#ActionEnum.Edit %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></dt>
            <dd>价格：<%#Eval("sell_price")%>&nbsp; 人气：<%#Eval("click")%></dd>
            ...
            <dd class="btns">
              <span class="right">
                ...
                <asp:ImageButton ID="ibtnHot" CommandName="ibtnHot" runat="server" ImageUrl='<%# Convert.ToInt32(Eval("is_hot")) == 1 ? "../images/ico-3.png" : "../images/ico-3_.png"%>' ToolTip='<%# Convert.ToInt32(Eval("is_hot")) == 1 ? "取消热门" : "设置热门"%>' />
                <asp:ImageButton ID="ibtnSlide" CommandName="ibtnSlide" runat="server" ImageUrl='<%# Convert.ToInt32(Eval("is_slide")) == 1 ? "../Images/ico-4.png" : "../Images/ico-4_.png"%>' ToolTip='<%# Convert.ToInt32(Eval("is_slide")) == 1 ? "取消幻灯片" : "设置幻灯片"%>' />
                <a href="photo_edit.aspx?channel_id=<%#this.channel_id %>&action=<%#ActionEnum.Edit %>&id=<%#Eval("id")%>"><img src="../images/ico-6.png" style="vertical-align:middle;" title="修改" /></a>
              </span>
